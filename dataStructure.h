...
void displayScore(List *ls) {
  ...
  while (tmp != NULL) {
    cout << tmp->q.date << "\t" << tmp->q.studentName << "\t" << tmp->q.studentScore << "\t" << tmp->q.duration << endl;
    tmp = tmp->next;
  }
}

void addToScoreList(List *ls, string studentName, int studentScore, double duration, string date) {
  Element *e;
  e = new Element;
...
inline void studentOpt(List *mainList) {
  ...
  case 1:
    ...
    takeTest(randomQuestionList, accuracyList);
    studentOpt(mainList);
    break;
  case 2:
    testTakerMenu();
